...
map:
    addi sp, sp, -24 # [+] we change the decrement amount of the sp in order to save the temp register we used before function call
    sw ra, 0(sp)
    sw s1, 4(sp)
    sw s0, 8(sp)
...
mapLoop:
    ...
    lw t1, 0(s0)        # [1] s0 is the address of the first struct, not the first array pointer.
    lw t2, 4(s0)        # load the size of the node's array into t2

    slli t3, t0, 2      # [2] note that the size of an element is 4-bytes not 1-byte
    add t1, t1, t3      # offset the array address by the count
    lw a0, 0(t1)        # load the value at that address into a0
    
    sw t0, 12(sp)       # [3] we save the temp register on the stack because it may changed in the function we wanna call
    sw t1, 16(sp)
    sw t2, 20(sp)
    jalr s1             # call the function on that value.
    lw t0, 12(sp)       # [4] restore
    lw t1, 16(sp)
    lw t2, 20(sp)
    ...
    mv a1, s1           # [5] put the address of the function back into a1 to prepare for the recursion
                        # the word in s1 is already the address of the function, so we just mv it to a1
